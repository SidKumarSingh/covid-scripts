...
#####################################
# Changelog:                        
# 04-Jun-2020: Passing cd_lookup to module
#####################################
import pandas as pd

def __find_dt(df, arglist):
    df_app = {'Metric':[], 'Date':[]}
    for arg in arglist:
...
def get_ox_data(data_ts, cd_lookup):
    ...
    dz = ts.groupby('Country_Region').apply(__find_dt, arglist = [{'cnt_col':'Confirmed', 'dt_col':'Date', 'res_col':'First_Case', 'dt_format':'%Y-%m-%d'}])
    ...
    cn_grp = data.loc[dz.index.unique().tolist()].reset_index().groupby('CountryCode').apply(__find_dt, arglist = [
    ...
    age_data = act_data.groupby('CountryCode').apply(__get_age).reset_index(level=1,drop=True).set_index('Metric', append=True)
    ...
    age_data = age_data.groupby('Metric').apply(__age_bin).set_index(['CountryCode','Metric'])
